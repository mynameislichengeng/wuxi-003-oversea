...
import cn.hugo.android.scanner.decode.CaptureActivityHandler2;
 ...
 * @author Sean Owen
 */
public abstract class CaptureActivity2 extends BATMicroActivity implements SurfaceHolder.Callback, View.OnClickListener {
	...
	/**
	 * 【辅助解码的参数(用作MultiFormatReader的参数)】 字符集，告诉扫描器该以何种字符集进行解码 对应于DecodeHintType.CHARACTER_SET类型 参考DecodeThread构造器如下代码：hints.put(DecodeHintType.CHARACTER_SET,
	 * characterSet);
	 */
	...
	public View llView, tvBottom;
	public void doOnCreatAction(int layoutId,Bundle savedInstanceState){
//		super.onCreate(savedInstanceState);
		Window window = getWindow();
		window.addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
//		setContentView(R.layout.capture);
		setMainView(layoutId);
		...
		llView = this.findViewById(R.id.llBtm);
		tvBottom = this.findViewById(R.id.capture_bottom_hint);
	}

	public void doOnCreateAction(Bundle savedInstanceState) {
		doOnCreatAction(R.layout.capture, savedInstanceState);
	}

	protected void doOnResumeAction() {
	...
	protected void doOnPauseAction() {
		if (handler != null) {
...
//		super.onDestroy();
	...
	public boolean doOnKeyDownAction(int keyCode, KeyEvent event) {
		switch (keyCode) {
	...
	public void doOnActivityResultAction(int requestCode, int resultCode, Intent intent) {
			...
			case REQUEST_CODE:
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						BitmapDecoder decoder = new BitmapDecoder(CaptureActivity2.this);
...
//			data.putExtra(InputInfoCameraActivity.IS_SUCCESS, InputInfoCameraActivity.TO_RESULT);
//			data.putExtra("result", ResultParser.parseResult(lastResult).toString());
//			setResult(RESULT_OK, data);
			
			//XXX 返回给子类
			Response response = new Response();
			response.code = 0;
			response.result =  ResultParser.parseResult(lastResult).toString();
			display(response);
//			Intent intent = new Intent();
//			intent.putExtra(InputInfoCameraActivity.IS_SUCCESS, String.valueOf(response));
