...
package com.wizarpos.pay.view.precent;
 ...
 * <p/>
 * <p>
 * This class collects utility methods that are involved in extracting percentage based dimension attributes and applying them to ViewGroup's children. If you
 * would like to implement a layout that supports percentage based dimensions, you need to take several steps:
 * <p/>
 * <ol>
 ...
 * <li>In your {@code LayoutParams(Context c, AttributeSet attrs)} constructor create an instance of {@link PercentLayoutInfo} by calling
 * {@link PercentLayoutHelper#getPercentLayoutInfo(Context, AttributeSet)}. Return this object from
 * {@code public PercentLayoutHelper.PercentLayoutInfo getPercentLayoutInfo()} method that you implemented for
 * {@link android.support.percent.PercentLayoutHelper.PercentLayoutParams} interface.
 * <li>Override {@link ViewGroup.LayoutParams#setBaseAttributes(TypedArray, int, int)} with a single line implementation {@code PercentLayoutHelper.fetchWidthAndHeight(this, a,
 * widthAttr, heightAttr);}
 * <li>In your ViewGroup override {@link ViewGroup#generateLayoutParams(AttributeSet)} to return your LayoutParams.
 * <li>In your {@link ViewGroup#onMeasure(int, int)} override, you need to implement following pattern:
 * 
 ...
 * </pre>
 * 
 * <li>In your {@link ViewGroup#onLayout(boolean, int, int, int, int)} override, you need to implement following pattern:
 * 
 * <pre class="prettyprint">
...
public class PercentLayoutHelper {
	...
	/**
	 * Helper method to be called from {@link ViewGroup.LayoutParams#setBaseAttributes} override that reads layout_width and layout_height attribute values
	 * without throwing an exception if they aren't present.
	 */
	...
	/**
	 * Constructs a PercentLayoutInfo from attributes associated with a View. Call this method from {@code LayoutParams(Context c, AttributeSet attrs)}
	 * constructor.
	 */
	...
	private static PercentLayoutInfo.PercentVal getPercentVal(String percentStr, boolean isOnWidth) {
		...
		if (!matcher.matches()) { throw new RuntimeException("the value of layout_xxxPercent invalid! ==>" + percentStr); }
	...
	/**
	 * Iterates over children and restores their original dimensions that were changed for percentage values. Calling this method only makes sense if you
	 * previously called {@link PercentLayoutHelper#adjustChildren(int, int)}.
	 */
	 ...
	 * <p/>
	 * If you are building a layout that supports percentage dimensions you are encouraged to take advantage of this method. The developer should be able to
	 * specify that a child should be remeasured by adding normal dimension attribute with {@code wrap_content} value. For example he might specify child's
	 * attributes as {@code app:layout_widthPercent="60%p"} and {@code android:layout_width="wrap_content"}. In this case if the child receives too little
	...
	public static class PercentLayoutInfo {
		...
		public void fillLayoutParams(ViewGroup.LayoutParams params, int widthHint, int heightHint) {
			...
			/*
			 * if (widthPercent >= 0) { params.width = (int) (widthHint * widthPercent); } if (heightPercent >= 0) { params.height = (int) (heightHint *
			 * heightPercent); }
			 */
		...
		public String toString() {
			return String.format("PercentLayoutInformation width: %f height %f, margins (%f, %f, " + " %f, %f, %f, %f)", widthPercent, heightPercent,
					leftMarginPercent, topMarginPercent, rightMarginPercent, bottomMarginPercent, startMarginPercent, endMarginPercent);

		}

		/**
		 * Restores original dimensions and margins after they were changed for percentage based values. Calling this method only makes sense if you previously
		 * called {@link PercentLayoutInfo#fillMarginLayoutParams}.
		 */
		...
		/**
		 * Restores original dimensions after they were changed for percentage based values. Calling this method only makes sense if you previously called
	 ...
	 * <p/>
	 * Your {@code LayoutParams} subclass should contain an instance of {@code PercentLayoutInfo} and the implementation of this interface should be a simple
	 * accessor.
	 */
