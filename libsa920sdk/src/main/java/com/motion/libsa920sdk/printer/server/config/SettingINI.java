package com.motion.libsa920sdk.printer.server.config;


import android.content.Context;
import android.text.TextUtils;

import com.motion.libsa920sdk.printer.server.internal.Convenience;
...
public class SettingINI {
    ...
    public static String getCommonSettingPath(Context context) {
        String settingIniFile = getAppDataFolder(context) + "/" + SettingINI.FILENAME;
        return settingIniFile;
    }

    public static boolean saveCommSettingToFile(Context context, final String fileName, final CommSetting commsetting) {
        IniFile ini;
        ...
        bDone &= ini.write(TagHost, Convenience.getHost(context, commsetting));
        bDone &= ini.write(TAG_ENABLE_PROXY, String.valueOf(commsetting.isEnableProxy()));
        return bDone;
    }

    public static CommSetting getCommSettingFromFile(Context context, final String fileName) {
        IniFile ini;
        ini = new IniFile(fileName);
        ini.setSection(SectionComm);
        ...
        Convenience.setHost(context, commsetting, ini.read(TagHost, Deft));
