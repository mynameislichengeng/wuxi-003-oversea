...
public class TenpayTransactionImpl extends OnlinePaymentTransactionImpl {
	private LooperQueryerTransactionState queryer;

	public TenpayTransactionImpl(Context context) {
		super(context);
	}

	protected String getKey(String msg) throws Exception {
		String key = Des3.encode(msg);
		return key;
	}
	...
	public void printTransInfo() {
		...
		String printString = "";
		if(isMixTransaction()){
			printString += builder.center(builder.bold("组合支付|QQ钱包支付"));
		}else{
			printString += builder.center(builder.bold("QQ钱包支付"));
		}
		...
		if (!TextUtils.isEmpty(transactionInfo.getThirdTradeNo())) {
			printString += "QQ钱包交易号：" + builder.br() + transactionInfo.getThirdTradeNo() + builder.br();
		}
		if(isMixTransaction()){
