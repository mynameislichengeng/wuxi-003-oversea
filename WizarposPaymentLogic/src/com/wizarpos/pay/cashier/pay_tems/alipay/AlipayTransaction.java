...
public class AlipayTransaction extends OnlinePaymentTransactionImpl {
	...
	protected String getKey() throws Exception {
		String key = Des3.encode(AppConfigHelper.getConfig(AppConfigDef.alipay_key));
		return key;
	}
	...
	public void printTransInfo() {
		...
		if(isMixTransaction()){
			sb.append(builder.center(builder.bold("组合支付|支付宝支付")));
		}else{
			sb.append(builder.center(builder.bold("支付宝支付")));
		}
		...
		if (!TextUtils.isEmpty(transactionInfo.getThirdTradeNo())) {
			sb.append("支付宝交易号：").append(builder.br()).append(transactionInfo.getThirdTradeNo()).append(builder.br());
		}
		if(isMixTransaction()){
